...
/// [LZ4DecompressorWithLength] and is NOT compatible with any other
/// decompressors in lz4-java or any other lz4 tools.  This class deliberately
/// does not extend [LZ4Compressor] because they are not interchangable.
public class LZ4CompressorWithLength {
    ...
    /// [compress(src, 0, src.length)][#compress(byte[], int, int)].
    ///
    ...
    /// Convenience method, equivalent to calling
    /// [compress(src, 0, src.length, dest, 0)][#compress(byte[], int, int, byte[], int)].
    ///
    /// @param src  the source data
    ...
    /// Convenience method, equivalent to calling
    /// [compress(src, srcOff, srcLen, dest, destOff, dest.length - destOff)][#compress(byte[], int, int, byte[], int, int)].
    ///
    /// @param src     the source data
    /// @param srcOff  the start offset in src
    /// @param srcLen  the number of bytes to compress

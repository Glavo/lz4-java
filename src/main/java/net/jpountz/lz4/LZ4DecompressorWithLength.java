...
/// Convenience class to decompress data compressed by [LZ4CompressorWithLength].
...
public class LZ4DecompressorWithLength {
    ...
    /// [decompress(src, 0, dest, 0)][#decompress(byte[], int, byte[], int)].
    ///
    /// @param src  the compressed data
    ...
    /// @return the number of bytes read to restore the original input (when [LZ4FastDecompressor] is used), or the number of decompressed bytes (when  [LZ4SafeDecompressor] is used)
    ...
    /// @param src     the compressed data
    /// @param srcOff  the start offset in src
    /// @param srcLen  the exact size of the compressed data (ignored when [LZ4FastDecompressor] is used)
    /// @param dest    the destination buffer to store the decompressed data
    /// @param destOff the start offset in dest
    /// @return the number of bytes read to restore the original input (when [LZ4FastDecompressor] is used), or the number of decompressed bytes (when  [LZ4SafeDecompressor] is used)
    ...
    /// Convenience method, equivalent to calling
    /// [decompress(src, 0)][#decompress(byte[], int)].
    ///
    /// @param src the compressed data
